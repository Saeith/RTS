...
namespace RTSEngine
{
    public class Building : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            { //if this is not a free building (belongs to a faction)
            ...
            { //if this is a free building
                ...
                if (PlacedByDefault == false) //if the building isn't to be placed by default
                    PlaceBuilding();
            }
            ...
            {
                ToggleConstructionObj(false); //disable the construction model
                OnDefaultBuildingInit(); //init placed by default building
            }
            else
            {//If the building requires construction:
        ...
        public void PlaceBuilding()
        {
            ...
            //if this is not a free building:
            if (FreeBuilding == false)
            {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            { //if the building is still not placed.
                //it's okay to place traps next to each other
        ...
        void OnFactionBuildingInit()
        {
            ...
            { //if the building is already placed in the map
        ...
        public void AddHealthLocal(float Value, GameObject Source) //changes the health for the local player.
        {
            ...
            else
            {
                ...
                if (GameManager.MultiplayerGame == false)
                { //if it's a single player game and this is not a free building
                    if (FactionID != GameManager.PlayerFactionID && FreeBuilding == false)
                    {
                ...
                if (GameManager.MultiplayerGame == false)
                {
                    ...
                    if (FactionID != GameManager.PlayerFactionID)
                    {
                        ...
                        if (TaskMgr != null)
                        {
                            ...
                            FactionMgr.ArmyMgr.ReloadArmyUnitsPriority(TaskMgr, false);
                            //inform the NPC unit spawner:
                            FactionMgr.UnitSpawner.ReloadTaskLauncherLists(TaskMgr, false);

                            //If the building belongs to one of the lists below, remove it from them:
                            if (FactionMgr.BuildingMgr.BuilderCreators.Contains(TaskMgr))
                            {
                                FactionMgr.BuildingMgr.BuilderCreators.Remove(TaskMgr);
                            }
                            if (FactionMgr.ResourceMgr.CollectorsCreators.Contains(TaskMgr))
                            {
                                FactionMgr.ResourceMgr.CollectorsCreators.Remove(TaskMgr);
                            }
                        }
        ...
        {
            if (PlaceNearResource == false)
            { //if we can place the building free from being in range of a resource
                return true;
            }
            else
            {
        ...
        public void CheckBuildingUpgrade()
        {
            ...
            if (ResourceMgr.CheckResources(BuildingUpgradeResources, FactionID, Ratio))
