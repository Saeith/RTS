...
namespace RTSEngine
{
	public class CustomEvents : MonoBehaviour {
		...
		public delegate void TaskEventHandler (TaskLauncher TaskComp, int TaskID);
		...
		//Task Events:
		public void OnTaskLaunched (TaskLauncher TaskComp, int TaskID) //called when a task launcher launches a task
		{
			if (DebugEnabled == true) {
				Debug.Log ("Task Launcher launched a task.");
			}
			TaskLaunched (TaskComp, TaskID);
		}
		public void OnTaskCanceled(TaskLauncher TaskComp, int TaskID) //called when a task launcher cancels a task
        {
			if (DebugEnabled == true) {
				Debug.Log ("Task Launcher canceled a pending task.");
			}
			TaskCanceled(TaskComp, TaskID);
        }
		public void OnTaskCompleted(TaskLauncher TaskComp, int TaskID) //called when a task launcher completes a task
        {
			if (DebugEnabled == true) {
				Debug.Log ("Task Launcher completed a task.");
			}
			TaskCompleted(TaskComp, TaskID);
        }
