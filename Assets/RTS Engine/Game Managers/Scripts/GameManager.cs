...
namespace RTSEngine
{
    ...
    public class GameManager : MonoBehaviour
    {
            ...
            { //If there's actually a network map manager, it means that the map was loaded from the multiplayer menu, meaning that this is a MP game.
                ...
                if (FactionLobbyInfos.Count <= Factions.Count)
                {
                    ...
                    for (int i = 0; i < FactionLobbyInfos.Count; i++)
                    {
                        ...
                        Factions[i].TypeInfo = NetworkMgr_UNET.Maps[ThisFaction.MapID].FactionTypes[ThisFaction.MapID];
                    ...
                    for (int i = 0; i < SinglePlayerMgr.Factions.Count; i++)
                    {
                        ...
                        if (Factions[i].TypeInfo != null)
                        {
                            ...
                            { //if the faction to a certain type
                                ...
                                GameObject Capital = Instantiate(Factions[i].TypeInfo.CapitalBuilding.gameObject);
            ...
            if (Factions.Count > 0)
            {
                ...
                for (int i = 0; i < Factions.Count; i++)
                {
                    ...
                    for (int j = 0; j < ResourceMgr.FactionResourcesInfo[i].ResourcesTypes.Length; j++)
                    {
                        ...
                        ResourceMgr.FactionResourcesInfo[i].ResourcesTypes[j].TypeInfo = ResourceMgr.ResourcesInfo[j].TypeInfo; //Name of the resource
                        ResourceMgr.FactionResourcesInfo[i].ResourcesTypes[j].Amount = ResourceMgr.ResourcesInfo[j].TypeInfo.StartingAmount; //Starting amount of the resource for each team.
                    }
        ...
        public void ClearNPCManagers()
        {
            ...
            foreach (NPCBuildingPlacement BuildingMgr in NPCBuildingMgrs)
            {
                DestroyImmediate(BuildingMgr);
            }

            NPCResource[] NPCResourceMgrs = FindObjectsOfType(typeof(NPCResource)) as NPCResource[];
            foreach (NPCResource ResourceMgr in NPCResourceMgrs)
            {

                DestroyImmediate(ResourceMgr);
            }

            NPCUnitSpawner[] NPCUnitSpawners = FindObjectsOfType(typeof(NPCUnitSpawner)) as NPCUnitSpawner[];
            foreach (NPCUnitSpawner UnitSpawner in NPCUnitSpawners)
            {

                DestroyImmediate(UnitSpawner);
            }
        }
