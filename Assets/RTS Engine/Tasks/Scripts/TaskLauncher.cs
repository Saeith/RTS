...
/* Task script created by Oussama Bouanani, SoumiDelRio.
...
namespace RTSEngine
{
    public class TaskLauncher : MonoBehaviour
    {
        ...
        public Vector3 GetSpawnPosition (bool FlyingUnit = false)
        {
            if(TaskHolder == TaskHolders.Building) //if the task holder is a building
            {
                return new Vector3(RefBuilding.SpawnPosition.position.x, TerrainMgr.SampleHeight(RefBuilding.SpawnPosition.position, FlyingUnit), RefBuilding.SpawnPosition.position.z); //return the building's assigned spawn position
            }
        ...
        void SetTaskTypes()
        {
            ...
            //if the faction is NPC and the task holder is a building, alert the NPC Army manager that a new building has been added, possibility of creating of army units/units that the NPC spawner need:
            if (FactionID != GameManager.PlayerFactionID && GameManager.MultiplayerGame == false)
            {
                ...
                {
                    //then make sure to proceed only if the building is to be placed by default as the NPC Building Placement will handle this for other buildings:
                    if (RefBuilding.PlacedByDefault == false)
                        return;
                }

                //inform the NPC unit spawner:
                FactionMgr.UnitSpawner.ReloadTaskLauncherLists(this, true);
                //inform the npc army:
        ...
        void UpdateTasks()
        {
            ...
            else
            {
                ...
                if (TasksList[TasksQueue[0].ID].TaskCompletedAudio != null)
                {
                    AudioManager.PlayAudio(GameMgr.GeneralAudioSource.gameObject, TasksList[TasksQueue[0].ID].TaskCompletedAudio, false); //Play the audio clip
                }

                //delegate event:
                if(GameMgr.Events)
                    GameMgr.Events.OnTaskCompleted(this, TasksQueue[0].ID);
        ...
        void OnUpgradeTaskCompleted()
        {
            ...
            if (TasksList[TasksQueue[0].ID].CurrentUpgradeLevel < TasksList[TasksQueue[0].ID].UnitCreationSettings.Upgrades.Count)
            {
                ...
                //if it's a NPC faction and this upgraded task is a create unit task + the task launcher is attached to a building
                if (FactionID != GameManager.PlayerFactionID && GameManager.MultiplayerGame == false && TasksList[TasksQueue[0].ID].TaskType == TaskManager.TaskTypes.CreateUnit && TaskHolder == TaskHolders.Building)
                {
                    //inform the NPC unit spawner:
                    FactionMgr.UnitSpawner.ReloadTaskLauncherLists(this, false);
                    FactionMgr.UnitSpawner.ReloadTaskLauncherLists(this, true);
            ...
            if (TasksList[TasksQueue[0].ID].TaskType == TaskManager.TaskTypes.CreateUnit)
            {
                ...
                if (GameManager.MultiplayerGame == false) //if this is a single player game
                {
                    ...
                    if (Cancel == false) //if the task is not to be cancelled
                    {
                        ...
                        Unit NewUnit = Instantiate(UnitPrefab.gameObject, GetSpawnPosition(UnitPrefab.FlyingUnit), UnitPrefab.transform.rotation).GetComponent<Unit>();
                        ...
                        NewUnit.CreatedBy = (TaskHolder == TaskHolders.Building) ? RefBuilding : null; //only assign the created by attribute if the unit was created by a building
                ...
                else
                {
                    ...
                    NewInputAction.InitialPos = GetSpawnPosition(UnitPrefab.FlyingUnit);
        ...
        public void CancelInProgressTask(int ID)
        {
            ...
            if (TasksQueue.Count > ID && ID >= 0)
            {
                ...
                //custom events
                if(GameMgr.Events)
                    GameMgr.Events.OnTaskCanceled(this, TasksQueue[ID].ID);
