...
namespace RTSEngine
{
	public class Unit : MonoBehaviour {
        ...
        public void CheckUnitPathLocal (Vector3 TargetPos, GameObject TargetObj, Vector3 LookAt, float StoppingDistance, InputTargetMode TargetMode)
		{
            ...
            NavAgent.SetDestination((FlyingUnit == true) ? new Vector3 (TargetPos.x, TerrainMgr.SampleHeight(TargetPos, true), TargetPos.z) : TargetPos); //calculate the path here
                ...
                if (AttackMgr) {
					if (GameManager.MultiplayerGame == false) {
						...
						if (FactionID != GameManager.PlayerFactionID) {
                            ...
                            if (TaskMgr != null)
                            {
                                //alert the NPC Army manager that a task launcher has been destroyed , possibility of creating of army units may be decreasd:
                                FactionMgr.ArmyMgr.ReloadArmyUnitsPriority(TaskMgr, false);
                                //inform the NPC unit spawner:
                                FactionMgr.UnitSpawner.ReloadTaskLauncherLists(TaskMgr, false);

                                //If the task launcher belongs to one of the lists below, remove it from them:
