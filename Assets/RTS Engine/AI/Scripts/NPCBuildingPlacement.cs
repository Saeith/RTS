...
namespace RTSEngine
{
    public class NPCBuildingPlacement : MonoBehaviour
    {
        ...
        [HideInInspector]
        public List<TaskLauncher> BuilderCreators = new List<TaskLauncher>(); //This will store the possible task launchers that can create builder type units.
                ...
                if (PendingBuildings[0].CenterID < FactionMgr.BuildingCenters.Count)
                {
                    if (FactionMgr.BuildingCenters[PendingBuildings[0].CenterID] != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (PendingBuildings[0].BuildingToAdd.IsBuildingOnMap() && PendingBuildings[0].BuildingToAdd.CanPlace == true && PendingBuildings[0].BuildingToAdd.IsBuildingInRange() == true && Delay <= 0)
                            { //If the building can be placed at its current position
        ...
        void StopPlacingBuilding()
        {
            ...
            //if the building to add has a task launcher:
            if (PendingBuildings[0].BuildingToAdd.TaskMgr)
            {
                //If the building can produce resource collectors, remove it from that list:
                if (FactionMgr.ResourceMgr.CollectorsCreators.Contains(PendingBuildings[0].BuildingToAdd.TaskMgr))
                {
                    FactionMgr.ResourceMgr.CollectorsCreators.Remove(PendingBuildings[0].BuildingToAdd.TaskMgr);
                }

                //If the building can produce builders, remove it from that list:
                if (BuilderCreators.Contains(PendingBuildings[0].BuildingToAdd.TaskMgr))
                {
                    BuilderCreators.Remove(PendingBuildings[0].BuildingToAdd.TaskMgr);
                }
            }
        ...
        public Resource GetBorderResource(string ResourceName, Border RefBorder)
        {
            int i = 0;
            while (i < RefBorder.ResourcesInRange.Count)
            { //go through all the resources inside the assigned border
                if (RefBorder.ResourcesInRange[i].Name == ResourceName)
                { //if this resource matches the resource type we're looking for
                    return RefBorder.ResourcesInRange[i]; //return this resource
                }
                i++;
            }
        ...
        {
            if (Code == BuildingCenter.Code)
            { //if the building to add is a building center 
            ...
            else
            {
                ...
                while (j < FactionMgr.BuildingCenters.Count)
                {
                    ...
                    if (FactionMgr.BuildingCenters[j].CurrentCenter.AllowBuildingInBorder(Code) == true)
                    { //make sure it allows this building under its territory
                        return j;
                    }
                    j++;
                }
        ...
        public void AskForBuildingResources(Building Building)
        {
            ...
            for (int i = 0; i < Building.BuildingResources.Length; i++)
            {
                ...
                if (Building.Code != BuildingCenter.Code && Amount == 0)
                { //no point in asking again for building centers, once is enough.
        ...
        public void AddBuildingToList(Building BuildingToAdd, int CenterID, GameObject BuildAround)
        {
            ...
            if (BuildAround != null)
            { //but sometimes we need a custom construction point
            ...
            Building BuildingClone = Instantiate(BuildingToAdd, SpawnPos, BuildingToAdd.transform.rotation).GetComponent<Building>();
        ...
        public Building GetBuildingByUnit(Unit NeededUnit)
        {
            ...
            while (i < AllBuildings.Count)
            {
                ...
                if (AllBuildings[i].TaskMgr != null)
                {
                    int j = 0;
                    //If the building has tasks
                    while (j < AllBuildings[i].TaskMgr.TasksList.Count)
                    {

                        //only buildings that have space for minimum a new task:
                        if (AllBuildings[i].TaskMgr.TasksQueue.Count < AllBuildings[i].TaskMgr.MaxTasks)
                        {

                            if (AllBuildings[i].TaskMgr.TasksList[j].TaskType == TaskManager.TaskTypes.CreateUnit)
                            {

                                //If the current building has a task that allows to produce the needed unit, then return its ID.
                                if (AllBuildings[i].TaskMgr.TasksList[j].UnitCreationSettings.Prefabs[0].Code == NeededUnit.Code)
                                {
                                    return AllBuildings[i];
                                }

                                if (AllBuildings[i].TaskMgr.TasksList[j].UnitCreationSettings.Upgrades.Count > 0)
                                {

                                    int k = 0;
                                    while (k < AllBuildings[i].TaskMgr.TasksList[j].UnitCreationSettings.Upgrades.Count)
                                    {
                                        if (AllBuildings[i].TaskMgr.TasksList[j].UnitCreationSettings.Upgrades[k].TargetPrefabs[0].Code == NeededUnit.Code)
                                        {
                                            return AllBuildings[i];
                                        }
                                        k++;
        ...
        public int GetInProgressBuilders(Building Target)
        {
            ...
            for (int i = 0; i < FactionMgr.Buildings.Count; i++)
            {
                if (FactionMgr.Buildings[i] != null)
                {
                    //if this building has a task launcher:
                    if (FactionMgr.Buildings[i].TaskMgr != null)
                    {
                        if (FactionMgr.Buildings[i].TaskMgr.BuilderUnits.Count > 0)
                        { //If the current building includes one or more tasks that produce builder units:

                            for (int j = 0; j < FactionMgr.Buildings[i].TaskMgr.TasksQueue.Count; j++)
                            {
                                if (FactionMgr.Buildings[i].TaskMgr.TasksQueue[j].TargetBuilding)
                                {
                                    ...
                                    if (FactionMgr.Buildings[i].TaskMgr.TasksQueue[j].TargetBuilding.Code == Target.Code)
                                    {
                                        Amount++;
                                    }
                                }
        ...
        public void SendCollectorsToBuild(Building TargetBuilding)
        {
            ...
            for (int i = 0; i < FactionMgr.BuildingCenters.Count; i++)
            {
                ...
                for (int j = 0; j < CurrentBorder.ResourcesInRange.Count; j++)
                {
                    ...
                    {
                        if (CurrentBorder.ResourcesInRange[j].gameObject.activeInHierarchy == true)
                        { //if the resource is active
                            if (CurrentBorder.ResourcesInRange[j].FactionID == FactionID)
                            { //If this resource is being collected by this faction:
                                if (CurrentBorder.ResourcesInRange[j].WorkerMgr.CurrentWorkers > 0)
                                { //If this resource has collectors.
                                    if (CurrentBorder.ResourcesInRange[j].WorkerMgr.GetWorker().BuilderMgr)
                                    { //If at least one of the collectors has a builder component.
        ...
        public bool CreateBuilders(Building Target, int Amount, Vector3 TargetArea)
        {
            ...
            if (Amount > 0 && BuilderCreators.Count > 0)
            {
                ...
                {
                    if (BuilderCreators[i].CanLaunchTask())
                    {
                        ...
                        if (Vector3.Distance(BuilderCreators[i].transform.position, TargetArea) < Distance || ID == -1)
                        {
                            ...
                            while (j < BuilderCreators[i].BuilderUnits.Count)
                            {
                                ...
                                if (FactionMgr.HasReachedLimit(BuilderCreators[i].TasksList[BuilderCreators[i].BuilderUnits[j]].UnitCreationSettings.Prefabs[0].Code) == false)
                                {
                                    ...
                                    if (BuilderCreators[i].TasksList[BuilderCreators[i].ResourceUnits[j]].UnitCreationSettings.Prefabs[0].gameObject.GetComponent<Attack>() || FactionMgr.ArmyMgr.CanCreateNonAttackUnit())
                                    {
                                        if (ResourceMgr.CheckResources(BuilderCreators[i].TasksList[BuilderCreators[i].BuilderUnits[j]].RequiredResources, FactionID, ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == true)
                                        {
                ...
                if (ID >= 0)
                {
                    while (Amount > 0 && ResourceMgr.CheckResources(BuilderCreators[ID].TasksList[TaskID].RequiredResources, FactionID, ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == true && BuilderCreators[ID].TasksQueue.Count < BuilderCreators[ID].MaxTasks)
                    {
                        //Check if we have enough resources to launch this task:
                        //Add the new task to the building's task queue.

                        GameMgr.TaskMgr.AddTask(BuilderCreators[ID], TaskID, -1, TaskManager.TaskTypes.CreateUnit);

                        Amount--;
                        CanProduceBuilders = true; //as soon as we're able to create even one unit then mark this as true.
                    }
                }
                else
                {
