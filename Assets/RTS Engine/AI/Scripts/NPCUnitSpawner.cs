...
namespace RTSEngine 
{
	public class NPCUnitSpawner : MonoBehaviour {
		...
		public class UnitsVars
		{
			...
			[HideInInspector]
			public List<TaskLauncher> SourceTaskLaunchers; //list of task launchers that can create this type of unit
			[HideInInspector]
                                ...
                                if (FactionMgr.HasReachedLimit(Units[i].Prefab.Code) == false)
                                {
                                    ...
                                    { //if we still have space in the population slots:
                                      //search for a building to create this unit:
                                        ...
                                        while (AmountLeft > 0 && j < Units[i].SourceTaskLaunchers.Count)
                                        { //go through the task launchers that can create this unit:
                                            while (AmountLeft > 0 && Units[i].SourceTaskLaunchers[j].TasksQueue.Count < Units[i].SourceTaskLaunchers[j].MaxTasks)
                                            {//if this still has space for one more task and we still haven't finished creating all units

                                                //check if we have enough resources:
                                                if (GameMgr.ResourceMgr.CheckResources(Units[i].SourceTaskLaunchers[j].TasksList[Units[i].TasksIDs[j]].RequiredResources, FactionID, FactionMgr.ResourceMgr.ResourceNeedRatio))
                                                {
                                                    //create a unit
                                                    GameMgr.TaskMgr.AddTask(Units[i].SourceTaskLaunchers[j], Units[i].TasksIDs[j], i, TaskManager.TaskTypes.CreateUnit);
                                                    AmountLeft--;
