...
namespace RTSEngine
{
    public class NPCArmy : MonoBehaviour
    {
        ...
        public class ArmyPrefabsVars
        {
            ...
            [HideInInspector]
            public List<TaskLauncher> SpawnedTaskLaunchers = new List<TaskLauncher>(); //a list of the spawned buildings that can produce the desired prefab.
        }
        ...
        public class ArmyVars
        {
            ...
            [HideInInspector]
            public TaskLauncher SourceTaskLauncher = null;
            [HideInInspector]
            public int PrefabID;
        ...
        public void CheckArmyUnitsPriority(int Min, int Max)
        {
            for (int i = Min; i < Max; i++)
            {
                ...
                {
                    if (ArmyUnits[i].Prefabs[j].SpawnedTaskLaunchers.Count > 0)
                    {
                ...
                ArmyUnits[i].SourceTaskLauncher = null;
            }
        }
        ...
        void Update()
        {
                ...
                if (ArmyUnits.Length > 0)
                {
                    ...
                    for (int i = 0; i < ArmyUnits.Length; i++)
                    {
                        ...
                        if (ArmyUnits[i].CurrentUnits.Count < ArmyUnits[i].AmountNeeded)
                        {
                            ...
                            {
                                int n = 0;
                                while (n < ArmyUnits[i].Prefabs[ArmyUnits[i].PrefabID].SpawnedTaskLaunchers.Count && ArmyUnits[i].CurrentUnits.Count + ArmyUnits[i].ProgressAmount < ArmyUnits[i].AmountNeeded)
                                {
                                    TaskLauncher TaskComp = ArmyUnits[i].Prefabs[ArmyUnits[i].PrefabID].SpawnedTaskLaunchers[n];
                                    ...
                                    if (TaskComp.ArmyUnits.Count > 0 && TaskComp.CanLaunchTask())
                                    {
                                        int Counter = 0;
                                        while (ID == -1 && Counter < TaskComp.ArmyUnits.Count)
                                        {
                                            //if this is the unit we're looking for.
                                            if (TaskComp.FactionID == FactionID && TaskComp.TasksList[TaskComp.ArmyUnits[Counter]].UnitCreationSettings.Prefabs[0].Code == ArmyUnits[i].Prefab.Code)
                                            {
                                                //make sure that the unit hasn't reached its limit:
                                                if (FactionMgr.HasReachedLimit(TaskComp.TasksList[TaskComp.ArmyUnits[Counter]].UnitCreationSettings.Prefabs[0].Code) == false)
                                                {
                                                    ID = TaskComp.ArmyUnits[Counter];
                                                }
                                            }
                                            Counter++;
                                        }
                                        if (ID >= 0)
                                        {
                                            ...
                                            while (GameMgr.ResourceMgr.CheckResources(TaskComp.TasksList[ID].RequiredResources, FactionID, GameMgr.ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == true && ArmyUnits[i].CurrentUnits.Count + ArmyUnits[i].ProgressAmount < ArmyUnits[i].AmountNeeded && ArmyUnits[i].ProgressAmount < (ArmyUnits[i].AmountNeeded - ArmyUnits[i].CurrentUnits.Count) && GameMgr.Factions[FactionID].CurrentPopulation < GameMgr.Factions[FactionID].MaxPopulation)
                                            {
                                                //Add the new task to the building's task queue
                                                GameMgr.TaskMgr.AddTask(TaskComp, ID, -1, TaskManager.TaskTypes.CreateUnit);

                                                //Start waiting for the new unit to be created:
                                                ArmyUnits[i].ProgressAmount++;
                                            }

                                            if (GameMgr.ResourceMgr.CheckResources(TaskComp.TasksList[ID].RequiredResources, FactionID, GameMgr.ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == false)
                                            { // No resources are available for this task:
                                                int RequiredAmount = ArmyUnits[i].AmountNeeded - (ArmyUnits[i].CurrentUnits.Count + ArmyUnits[i].ProgressAmount);
                                                for (int x = 0; x < TaskComp.TasksList[ID].RequiredResources.Length; x++)
                                                {
                                                    ...
                                                    FactionMgr.ResourceMgr.LookForResources(TaskComp.TasksList[ID].RequiredResources[x].Name, TaskComp.TasksList[ID].RequiredResources[x].Amount * RequiredAmount);
                            ...
                            //No building to create army units
                            if (ID == -1 && ArmyUnits[i].CurrentUnits.Count + ArmyUnits[i].ProgressAmount < ArmyUnits[i].AmountNeeded)
                            {
                                //We need to construct a building that can create the required army unit:
                                //look for the suitable building to build this unit if we don't already have it saved:
                                if (ArmyUnits[i].SourceTaskLauncher == null)
                                {
                                    Building TaskLauncherBuilding = FactionMgr.BuildingMgr.GetBuildingByUnit(ArmyUnits[i].Prefab);
                                    ArmyUnits[i].SourceTaskLauncher = (TaskLauncherBuilding != null) ? TaskLauncherBuilding.TaskMgr : null;
                                }

                                //if we have a source task launcher prefab then:
                                if (ArmyUnits[i].SourceTaskLauncher != null)
                                {
                                    //if it's attached to a building
                                    if (ArmyUnits[i].SourceTaskLauncher.GetComponent<Building>())
                                    { //attempt to construct it
                                        FactionMgr.BuildingMgr.AttemptToAddBuilding(ArmyUnits[i].SourceTaskLauncher.GetComponent<Building>(), false, null);
                                    }
                                }
                            }
                        }
            ...
            if (Attacking == true)
            { //If the faction is currently in war:
                if (TargetFaction != null)
                {
                    if (AttackTimer > 0)
                    {
                    ...
                    if (AttackTimer < 0)
                    {
                        ...
                        {
                            if (TargetBuilding == null)
                            { //If the target building is not valid
                              //Search for one:
                                SetTargetBuilding();
                            }
        ...
        public void LaunchAttack()
        {
            ...
            if (TargetFaction != null)
            { //if the target faction is a NPC (has a army manager)
                ...
                if (Army.Count > 0)
                {
                    for (int i = 0; i < Army.Count; i++)
                    {
                        ...
                        if (Army[i] != null)
                        {
                            ...
                            if (Army[i].AttackMgr.AttackTarget == null)
                            { //set the attacking support range that will call nearby units for support.
                    ...
                    {
                        if (FactionMgr.Healers[j].TargetUnit == null)
                        { //only move the unit if it has no target
