...
namespace RTSEngine
{
    public class NPCResource : MonoBehaviour
    {
        ...
        [HideInInspector]
        public List<TaskLauncher> CollectorsCreators = new List<TaskLauncher>(); //This will store the possible task launchers that can produce resource collectors
        ...
        public void Update()
        {
            ...
            if (CheckResources == true)
            {
                ...
                { //make sure that there are not other buildings that require construction
            ...
            if (DropOffControlDone == false)
            {
                ...
                if (DropOffControlTimer < 0 && FactionMgr.BuildingMgr.CheckBuildings == false)
                { //make sure that there are not other buildings that require construction
                    DropOffControl();
            ...
            if (AllUnitsUpgraded == false)
            {
                ...
                if (TaskUpgradeTimer < 0)
                {
                    ...
                    {
                        //Go through all the faction's task launchers
                        for (int i = 0; i < FactionMgr.TaskLaunchers.Count; i++)
                        {
                            if (FactionMgr.TaskLaunchers[i].CanLaunchTask())
                            { //making sure the task launcher can actually add tasks

                                //go through the tasks:
                                if (FactionMgr.TaskLaunchers[i].TasksList.Count > 0)
                                {
                                    for (int j = 0; j < FactionMgr.TaskLaunchers[i].TasksList.Count; j++)
                                    {
                                        if (FactionMgr.TaskLaunchers[i].TasksList[j].TaskType == TaskManager.TaskTypes.CreateUnit)
                                        { //if this task allows to create a unit.

                                            if (FactionMgr.TaskLaunchers[i].TasksList[j].UnitCreationSettings.Upgrades.Count > FactionMgr.TaskLaunchers[i].TasksList[j].CurrentUpgradeLevel)
                                            { //if this task has possible upgrades:
                                                AllUnitsUpgraded = false;

                                                if (FactionMgr.TaskLaunchers[i].TasksList[j].Active == false)
                                                {//if the task is not currently upgrading:

                                                    //Is there enough resources for this?
                                                    if (ResourceMgr.CheckResources(FactionMgr.TaskLaunchers[i].TasksList[j].UnitCreationSettings.Upgrades[FactionMgr.TaskLaunchers[i].TasksList[j].CurrentUpgradeLevel].UpgradeResources, FactionID, TaskUpgradeResourceRatio) == true)
                                                    {
                                                        if (FactionMgr.TaskLaunchers[i].MaxTasks > FactionMgr.TaskLaunchers[i].TasksQueue.Count)
                                                        { //make sure the building can still launch tasks

                                                            GameMgr.TaskMgr.AddTask(FactionMgr.TaskLaunchers[i], j, -1, TaskManager.TaskTypes.TaskUpgrade);
                                                        }
                                                    }
                                                }
                                            }
        ...
        public void CheckResourceCollection()
        {
            ...
            for (int i = 0; i < FactionMgr.BuildingCenters.Count; i++)
            {
                if (FactionMgr.BuildingCenters[i].CurrentCenter.ResourcesInRange.Count > 0 && FactionMgr.BuildingCenters[i].CurrentCenter.IsActive == true)
                { //If there are resources inside the building center's borders
                  //We'll go through all different types of resources:
                    for (int n = 0; n < ResourceMgr.ResourcesInfo.Length; n++)
                    {
                        string Name = ResourceMgr.ResourcesInfo[n].TypeInfo.Name;
                        int j = 0;
                        ...
                        {
                            if (FactionMgr.BuildingCenters[i].CurrentCenter.ResourcesInRange[j] != null)
                            { //if the resource still exists and it's active:
                ...
                for (int i = 0; i < FactionResources.Count; i++)
                {
                    ...
                    if (FactionResources[i].Resource != null)
                    {
                        ...
                        if (FactionResources[i].DropOff == null)
                        {
                            ...
                            { //if there are drop off buildings
                            ...
                            if (Found == false)
                            {
                                ...
                                {
                                    j = 0;
                                    while (j < FactionMgr.BuildingMgr.AllBuildings.Count)
                                    { //go through all the buildings prefabs:
                                        if (FactionMgr.BuildingMgr.AllBuildings[j].CanDrop(FactionResources[i].Resource.Name))
                                        { //if this building matches
        ...
        public bool HasDropOff(Resource Resource)
        {
            ...
            if (FactionResources.Count > 0)
            {
                while (Found == false && i < FactionResources.Count)
                {
                    ...
                    if (FactionResources[i].Resource == Resource)
        ...
        public void LookForResources(string Name, int Amount)
        {
            ...
            if (ID >= 0)
            { //Make sure that the resource name is valid.
                ...
                if (ResourceMgr.FactionResourcesInfo[FactionID].ResourcesTypes[ID].TargetAmount > ResourceMgr.FactionResourcesInfo[FactionID].ResourcesTypes[ID].Amount)
                {
                    ...
                    while (i < FactionMgr.BuildingCenters.Count && TargetResource == null)
                    {
                        if (FactionMgr.BuildingCenters[i])
                        {
                            if (FactionMgr.BuildingCenters[i].CurrentCenter)
                            {
                                ...
                                {
                                    if (FactionMgr.BuildingCenters[i].CurrentCenter.ResourcesInRange.Count > 0)
                                    { //If there are resources inside the building center's borders
                                        int j = 0;
                    ...
                    {//If no resources are found
                        ...
                        if (ResourceGenerators.Count > 0)
                        {
                            ...
                            {
                                if (ResourceGenerators[n].Name == Name)
                                { //resource generator building found:
                                    Found = true;
                                }
                                n++;
                            }
            ...
            if (FactionMgr.BuildingMgr.IsBeingBuilt(FactionMgr.BuildingMgr.BuildingCenter) == false)
            {
                ...
                if (FactionMgr.BuildingMgr.BuildingCenter != null && FactionMgr.BuildingCenters.Count > 0)
                {
                    ...
                    for (int i = 0; i < ResourceMgr.AllResources.Count; i++)
                    {
                        ...
                        {
                            if (ResourceMgr.AllResources[i].FactionID == -1)
                            { //Make sure no one else is exploiting this resouce 
        ...
        public int GetInProgressCollectors(Resource Target)
        {
            ...
            {
                if (CollectorsCreators[i].ResourceUnits.Count > 0)
                { //If the current building includes one or more tasks that produce collector units:
        ...
        public bool CreateCollectors(Resource Target, int Amount, Vector3 TargetPos)
        {
            ...
            if (Amount > 0 && CollectorsCreators.Count > 0)
            {
                ...
                for (i = 0; i < CollectorsCreators.Count; i++)
                {
                    ...
                    {
                        if (CollectorsCreators[i].CanLaunchTask())
                        {
                            ...
                            while (j < CollectorsCreators[i].ResourceUnits.Count)
                            {
                                ...
                                if (FactionMgr.HasReachedLimit(CollectorsCreators[i].TasksList[CollectorsCreators[i].ResourceUnits[j]].UnitCreationSettings.Prefabs[0].Code) == false)
                                {
                                    ...
                                    if (CollectorsCreators[i].TasksList[CollectorsCreators[i].ResourceUnits[j]].UnitCreationSettings.Prefabs[0].gameObject.GetComponent<Attack>() || FactionMgr.ArmyMgr.CanCreateNonAttackUnit())
                                    {
                                        if (ResourceMgr.CheckResources(CollectorsCreators[i].TasksList[CollectorsCreators[i].ResourceUnits[j]].RequiredResources, FactionID, ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == true)
                                        {
                ...
                if (ID >= 0)
                {
                    while (Amount > 0 && ResourceMgr.CheckResources(CollectorsCreators[ID].TasksList[TaskID].RequiredResources, FactionID, ResourceMgr.FactionResourcesInfo[FactionID].NeedRatio) == true && CollectorsCreators[ID].TasksQueue.Count < CollectorsCreators[ID].MaxTasks)
                    {
                        //Check if we have enough resources to launch this task:
                        //Add the new task to the launcher's task queue
                        GameMgr.TaskMgr.AddTask(CollectorsCreators[ID], TaskID, -1, TaskManager.TaskTypes.CreateUnit);
                        ...
                        if (CurrentBorder.ResourcesInRange[j].gameObject.activeInHierarchy == true) //if the resource is active
                        {
                            ...
                            if (CurrentBorder.ResourcesInRange[j].Name != TargetResource.Name)
                            {
                                ...
                                { //If this resource is being collected by this faction:
                                    if (CurrentBorder.ResourcesInRange[j].WorkerMgr.CurrentWorkers > Collectors)
                                    { //If this resource has the most collectors
